...
class MyService_No_Micro : Service(), SensorEventListener {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (turnOnNotificationFromHome == true) {
            Log.d(TAG, "onStartCommand: turn on noti")
            offService()
        }
            ...
            if (intent?.action == Intent.ACTION_BATTERY_CHANGED) {
                ...
                when (status) {
                    ...
                    BatteryManager.BATTERY_STATUS_DISCHARGING -> {
                        ...
                        stopChargerPhone()
                        WakeupPhone.foundPhone(this@MyService_No_Micro)
                    }
                }
            }
        }
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        if (SharePreferenceUtils.getRunningService() == Constant.Service.POCKET_MODE_RUNNING) {
            ...
            if (event?.sensor?.type == Sensor.TYPE_ACCELEROMETER) {
                ...
                if (acceleration > 10.5f && currentTime - mLastShakeTime > SHAKE_THRESHOLD) {
